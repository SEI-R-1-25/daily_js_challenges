...
function reverseUpcaseString(str) {
  ...
  for (let i = str.length - 1; i >= 0; i--) {
    reverse = reverse + str.charAt(i).toUpperCase()
  }
  return reverse
}
/*-----------------------------------------------------------------
...
function charCount(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    counts[currentLetter] = counts[currentLetter] + 1 || 1
  }
  return counts
}
/*-----------------------------------------------------------------
...
function primeFactors(integer) {
  const newArray = []
  let primeNum = 2
  if (integer < 2 || !Number.isInteger(integer)) return newArray
  while (isPrime(integer) === false) {
    if (Number.isInteger(integer / primeNum)) {
      newArray.push(primeNum)
      integer = integer / primeNum
    } else {
      primeNum += 1
    }
  }
...
function intersection(a, b) {
  ...
  newArr = [...new Set(a)].filter((x) => setB.has(x))
  console.log(newArr)
  return newArr
}
/*-----------------------------------------------------------------
...
function gridTrip(start, moves) {
  ...
  for (let i = 0; i < moves.length; i++) {
    switch (moves[i]) {
      case 'U':
        if (isNaN(moves[i + 2])) {
          x += parseInt(moves[i + 1])
        } else {
          x += parseInt(moves[i + 1] + moves[i + 2])
        }
        break
      case 'D':
        if (isNaN(moves[i + 2])) {
      ...
      case 'L':
        if (isNaN(moves[i + 2])) {
          y -= parseInt(moves[i + 1])
        } else {
          y -= parseInt(moves[i + 1] + moves[i + 2])
        }
        break
      case 'R':
        if (isNaN(moves[i + 2])) {
...
function totalTaskTime(taskArr, numThreads) {
  var time = 0,
    shortest,
    threads
  while (taskArr.length > numThreads) {
    threads = taskArr.splice(0, numThreads)
    shortest = Math.min(...threads)
    time += shortest
    threads = threads.map((t) => t - shortest).filter((t) => t)
    taskArr = threads.concat(taskArr)
  }
  return time + (taskArr.length ? Math.max(...taskArr) : 0)
}